...
batch_size = 16
learning_rate_schedule = {0: 0.01, 1000: 0.003, 2000: 0.001, 3000: 0.0003}
...
def build_model(n_channels, n_fbins, n_timesteps):
    ...
    l2c = nn.layers.Conv2DLayer(nn.layers.dropout(l1c, p=0.1), num_filters=32, filter_size=(1, 1),
                                W=nn.init.Orthogonal('relu'),
                                b=nn.init.Constant(0.1),
                                nonlinearity=nn.nonlinearities.rectify)

    l3c = nn.layers.Conv2DLayer(nn.layers.dropout(l2c, p=0.2), num_filters=64, filter_size=(1, 1),
                                W=nn.init.Orthogonal('relu'),
                                b=nn.init.Constant(0.1),
                                nonlinearity=nn.nonlinearities.rectify)
...
def build_updates(train_loss, model, learning_rate):
    updates = nn.updates.adadelta(train_loss, nn.layers.get_all_params(model.l_out), learning_rate)
    return updates
